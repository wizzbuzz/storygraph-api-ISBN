...
class UserParser:
    @staticmethod
    @parsing_exception 
    def parse_html(html,get_id, get_isbn):
        ...
        for book in books:
            ...
            if(get_isbn):
                book_isbn = soup.find('span', string=re.compile(r'isbn', re.IGNORECASE)).parent.text.split(" ")[1]
    ...
    @staticmethod
    def currently_reading(uname, cookie,get_id, get_isbn):
        content = UserScraper.currently_reading(uname,cookie)
        return UserParser.parse_html(content,get_id, get_isbn)

    @staticmethod
    def to_read(uname, cookie,get_id, get_isbn):
        content = UserScraper.to_read(uname,cookie)
        return UserParser.parse_html(content,get_id, get_isbn)

    @staticmethod
    def books_read(uname, cookie,get_id, get_isbn):
